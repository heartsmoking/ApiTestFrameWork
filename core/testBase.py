...
class BaseTest(requests.Session):
    ...
    def api_log(self, method, url, headers=None, params=None, json=None, cookies=None, file=None, code=None,
        ...
        logger.debug("上传附件为======>{}".format(file))
        logger.debug("Cookies====>{}".format(dumps(cookies, indent=4)))
        logger.info("接口响应状态码====>{}".format(code))
        logger.debug("接口响应头为====>{}".format(res_header))
        logger.debug("接口响应体为====>{}".format(res_text))
